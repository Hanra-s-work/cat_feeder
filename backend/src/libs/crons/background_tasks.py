...
class BackgroundTasks(metaclass=FinalClass):
    """_summary_
    ...
    def safe_add_task(self, func: Callable, args: Union[Tuple, None] = None, kwargs: Union[Dict, None] = None, trigger: Union[str, Any] = "interval", seconds: int = 5) -> Union[int, Job]:
        ...
        try:
            ...
            if data is None:
                self.disp.log_error(
                    "The add_task function returned None, which is unexpected.",
                    "safe_add_task"
                )
                return self.error
            return data
    ...
    def safe_start(self) -> int:
        ...
        try:
            ...
            if data is None:
                self.disp.log_error(
                    "The start function returned None, which is unexpected.",
                    "safe_start"
                )
                return self.error
            return data
    ...
    def safe_pause(self, pause: bool = True) -> int:
        ...
        """
        try:
            data = self.pause(pause=pause)
            if data is None:
                self.disp.log_error(
                    "The pause function returned None, which is unexpected.",
                    "safe_pause"
                )
                return self.error
            return data
    ...
    def safe_resume(self) -> int:
        ...
        try:
            ...
            if data is None:
                self.disp.log_error(
                    "The resume function returned None, which is unexpected.",
                    "safe_resume"
                )
                return self.error
            return data
    ...
    def safe_stop(self, wait: bool = True) -> int:
        ...
        """
        try:
            data = self.stop(wait=wait)
            if data is None:
                self.disp.log_error(
                    "The stop function returned None, which is unexpected.",
                    "safe_stop"
                )
                return self.error
            return data
