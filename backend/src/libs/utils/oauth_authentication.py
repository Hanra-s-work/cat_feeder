...
# LAST Modified: 9:56:14 27-11-2025
# DESCRIPTION: 
...
class OAuthAuthentication:
    ...
    def __init__(self, success: int = 0, error: int = 84, debug: bool = False) -> None:
        ...
        # -------------------------- Shared instances --------------------------
        self.database_link: SQL = self.runtime_manager_initialised.get(SQL)
        self.boilerplate_non_http_initialised: BoilerplateNonHTTP = self.runtime_manager_initialised.get(
            BoilerplateNonHTTP
        )
        self.boilerplate_responses_initialised: BoilerplateResponses = self.runtime_manager_initialised.get(
            BoilerplateResponses)
        self.boilerplate_incoming_initialised: BoilerplateIncoming = self.runtime_manager_initialised.get(
            BoilerplateIncoming)
        self.server_headers_initialised: ServerHeaders = self.runtime_manager_initialised.get(
            ServerHeaders)
    ...
    def _generate_oauth_authorization_url(self, provider: str) -> Union[int, str]:
        ...
        retrived_provider = self.database_link.get_data_from_table(
        ...
        columns = self.database_link.get_table_column_names(
        ...
        et_str = self.database_link.datetime_to_string(
        ...
        data.append(state)
        data.append(et_str)
        if self.database_link.insert_data_into_table(CONST.TAB_VERIFICATION, data, columns) == self.error:
            return self.error
        state += ":"
        state += provider
        if provider == "google":
    ...
    def _exchange_code_for_token(self, provider: str, code: str):
        ...
        retrieved_provider = self.database_link.get_data_from_table(
            CONST.TAB_USER_OAUTH_CONNECTION,
            "*",
            f"provider_name='{provider}'"
        )
        if isinstance(retrieved_provider, int):
            return self.boilerplate_responses_initialised.build_response_body(
    ...
    def _get_user_info(self, provider: str, access_token: str):
        ...
        retrieved_data = self.database_link.get_data_from_table(
            CONST.TAB_USER_OAUTH_CONNECTION,
            "*",
            f"provider_name='{provider}'"
        )
        self.disp.log_debug(
            f"Retrieved oauth provider data: {retrieved_data}", title)
    ...
    def _oauth_user_logger(self, user_info: Dict, provider: str, connection_data: list) -> Response:
        ...
        retrieved_user = self.database_link.get_data_from_table(
            CONST.TAB_ACCOUNTS,
            "*",
            f"email='{email}'"
        )
        ...
        if isinstance(retrieved_user, int) is False:
            ...
            if isinstance(retrieved_user, int):
                return self.boilerplate_responses_initialised.internal_server_error(title, None)
            ...
            if isinstance(self.database_link.get_data_from_table(
                ...
                columns.pop(0)
                self.disp.log_debug(f"Columns list = {columns}", title)
                if self.database_link.insert_data_into_table(
                    CONST.TAB_ACTIVE_OAUTHS,
                    connection_data,
                    columns
                ) == self.error:
                    return self.boilerplate_responses_initialised.internal_server_error(title, None)
            user_data = self.boilerplate_incoming_initialised.log_user_in(
                email
            )
        ...
        if self.database_link.insert_data_into_table(CONST.TAB_ACCOUNTS, user_data, columns) == self.error:
            return self.boilerplate_responses_initialised.internal_server_error(title, None)
        retrieved_user = self.database_link.get_data_from_table(
            CONST.TAB_ACCOUNTS,
            "*",
    ...
    def _handle_token_response(self, token_response: Dict, provider: str) -> Response:
        ...
        if not access_token:
            return self.boilerplate_responses_initialised.no_access_token(title, None)
        ...
        if provider == "github":
            ...
            expiration_date = self.database_link.datetime_to_string(
                new_time
            )
        ...
        self.disp.log_debug(
            f"Generated data for new oauth connexion user: {data}", title)
    ...
    def refresh_token(self, provider_name: str, refresh_link: str) -> Union[str, None]:
        ...
        if any(word in provider_name for word in ("google", "discord", "spotify")):
            ...
            self.disp.log_debug(
                f"Provider response: {provider_response}", title)
    ...
    async def oauth_callback(self, request: Request) -> Response:
        ...
        if isinstance(data, int):
            return self.boilerplate_responses_initialised.internal_server_error(title, None)
        if isinstance(self.database_link.drop_data_from_table(
            CONST.TAB_VERIFICATION,
            f"definition='{uuid_gotten}'"
        ), int) is False:
            return self.boilerplate_responses_initialised.internal_server_error(title, None)
    ...
    async def add_oauth_provider(self, request: Request, provider: str = "") -> Response:
        ...
        retrived_data = self.database_link.get_data_from_table(
            CONST.TAB_USER_OAUTH_CONNECTION,
            "*",
            f"provider_name='{provider}'"
        )
        ...
        if self.database_link.insert_data_into_table(CONST.TAB_USER_OAUTH_CONNECTION, data, columns) == self.error:
            return self.boilerplate_responses_initialised.internal_server_error(title, token)
        oauth: list = ["1"]
        column: list = ["oauth"]
    ...
    async def update_oauth_provider_data(self, request: Request, provider: str) -> Response:
        ...
        if not provider:
            return self.boilerplate_responses_initialised.provider_not_given(title, None)
        self.disp.log_debug(f"Provider: {provider}", title)
        token: str = self.boilerplate_incoming_initialised.get_token_if_present(
            request
        )
        ...
        columns: list = self.database_link.get_table_column_names(
            CONST.TAB_USER_OAUTH_CONNECTION
        )
        if isinstance(columns, int):
            return self.boilerplate_responses_initialised.internal_server_error(title, None)
    ...
    async def patch_oauth_provider_data(self, request: Request, provider: str) -> Response:
        ...
        request_body = await self.boilerplate_incoming_initialised.get_body(request)
        if not request_body:
            return self.boilerplate_responses_initialised.missing_variable_in_body(title, token)
        self.disp.log_debug(f"Request body: {request_body}", title)
        if "provider_name" in request_body:
            if self.boilerplate_non_http_initialised.update_single_data(
                CONST.TAB_USER_OAUTH_CONNECTION,
                "provider_name",
                "provider_name",
                provider,
        ...
        return HCI.success(body, content_type=HTTP_DEFAULT_TYPE, headers=self.server_headers_initialised.for_json())
    ...
    async def delete_oauth_provider(self, request: Request, provider: str) -> Response:
        ...
        if not provider:
            return self.boilerplate_responses_initialised.provider_not_given(
                title,
                None
            )
        self.disp.log_debug(f"Provider: {provider}", title)
        ...
        if self.boilerplate_non_http_initialised.is_token_admin(token) is False:
            self.disp.log_error("You're not admin.", title)
            return self.boilerplate_responses_initialised.insuffisant_rights(
                title,
                token
            )
        retrived_data = self.database_link.get_data_from_table(
        ...
        retrieved_service_id = self.database_link.get_data_from_table(
            CONST.TAB_SERVICES,
            "id",
            f"name='{provider}'"
        )
        ...
        if self.database_link.drop_data_from_table(
            CONST.TAB_ACTIVE_OAUTHS,
