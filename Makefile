...
# Requirements files
REQUIREMENTS_DIR	=	./backend/deps
REQUIREMENTS	=	$(REQUIREMENTS_DIR)/requirements.backend.txt
REQUIREMENTS_DEV	=	$(REQUIREMENTS_DIR)/requirements.dev.txt
REQUIREMENTS_TESTING	=	$(REQUIREMENTS_DIR)/requirements.testing.txt
...
# Search for pip version (bound to the venv)
MY_PIP_BIN	=	./$(ENV_NAME)/bin/python -m pip
...
install_dependencies: create_environement
	...
	$(MY_PIP_BIN) install --upgrade pip wheel setuptools && \
\
	$(MY_PIP_BIN) list && \
	echo -e "$(C_CYAN)Installing python dependencies$(C_RESET)" &&\
\
	$(MY_PIP_BIN) install -r $(REQUIREMENTS)

# Install the dev python dependencies
install_dev_dependencies: install_dependencies
	$(SILENT) echo -e "$(C_CYAN)Upgrading pip$(C_RESET)" && \
	. $(ENV_NAME)/bin/activate && \
\
	echo -e "$(C_CYAN)Installing python dependencies$(C_RESET)" &&\
	$(MY_PIP_BIN) install -r $(REQUIREMENTS_DEV)

# Install the test python dependencies
install_test_dependencies: install_dev_dependencies
	$(SILENT) echo -e "$(C_CYAN)Upgrading pip$(C_RESET)" && \
	. $(ENV_NAME)/bin/activate && \
\
	echo -e "$(C_CYAN)Installing python dependencies$(C_RESET)" &&\
	$(MY_PIP_BIN) install -r $(REQUIREMENTS_TESTING)
...
build_binary:
	...
	$(SILENT) mkdir -p "$(BINARY_DESTINATION_ONE)" "$(BINARY_DESTINATION_TWO)"
	$(SILENT) . ./$(ENV_NAME)/bin/activate && \
	pyinstaller \
				$(SRC)	\
...
tests_run: install_test_dependencies
...
.PHONY:
	all omega create_environement \
	install_dependencies install_dev_dependencies install_test_dependencies \
	activate_environement activate \
	build build_binary update_binary_location \
